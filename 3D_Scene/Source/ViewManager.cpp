...
{
	// if the camera object is valid, adjust the camera movement speed
	if (nullptr != g_pCamera)
	{
		// Pass -yoffset so that scrolling up (positive yoffset) increases movement speed
		// and scrolling down (negative yoffset) decreases movement speed
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// process camera panning left/right
	...
	// process camera vertical movement (upward and downward)
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS)
	{
		...
		// restore the default camera perspective parameters
		g_pCamera->Position = glm::vec3(0.0f, 5.0f, 12.0f);
		g_pCamera->Front = glm::vec3(0.0f, -0.5f, -2.0f);
		g_pCamera->Up = glm::vec3(0.0f, 1.0f, 0.0f);
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS)
	{
		...
		// change the camera settings to look directly horizontally at the 3D object
		g_pCamera->Position = glm::vec3(0.0f, 1.5f, 10.0f);
		g_pCamera->Up = glm::vec3(0.0f, 1.0f, 0.0f);
...
void ViewManager::PrepareSceneView()
{
	...
	else
	{
		...
		{
			scale = (float)WINDOW_HEIGHT / (float)WINDOW_WIDTH;
			projection = glm::ortho(-5.0f, 5.0f, -5.0f * scale, 5.0f * scale, 0.1f, 100.0f);
		}
		else if (WINDOW_WIDTH < WINDOW_HEIGHT)
		{
			scale = (float)WINDOW_WIDTH / (float)WINDOW_HEIGHT;
			projection = glm::ortho(-5.0f * scale, 5.0f * scale, -5.0f, 5.0f, 0.1f, 100.0f);
		}
		else
		{
